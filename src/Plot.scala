...
class MyPlot extends Activity with ActivityHelper {
    import helpers._
    ...
    protected override def onCreate(savedInstanceState : Bundle) {
        ...
        val card = movies.getFilmUpCard(movieId);

        (if (card != null) card.getAsString("plot") else null, card) match {
    ...
    def searchReview(replace : Boolean) {
        val intent = new Intent(this, classOf[SearchPlots]);
    ...
    def reviewLoaded(success : Boolean) {
        ...
        val card = Movies.getInstance(this).getFilmUpCard(movieId);

        setReviewText(card.getAsString("plot"));
    }
...
class SearchPlots extends ListActivity with ActivityHelper {
    import helpers._
    ...
    override def onListItemClick(l : ListView, v : View,
        ...
        movies.setFilmUpPlot(movieId, null);
